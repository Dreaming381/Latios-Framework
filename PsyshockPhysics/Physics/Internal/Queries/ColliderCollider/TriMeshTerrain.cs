...
namespace Latios.Psyshock
{
    internal static class TriMeshTerrain
    {
                                           ...
                                           out ColliderDistanceResult result)
        {
            ...
            DistanceBetweenAll(in terrain, in terrainTransform, in triMesh, in triMeshTransform, maxDistance, ref processor);
            ...
            var triangle        = PointRayTerrain.CreateLocalTriangle(ref terrain.terrainColliderBlob.Value,
                                                                      ...
                                                                      terrain.scale);
            hit                                  = TriangleTriMesh.DistanceBetween(in triMesh, in triMeshTransform, in triangle, in terrainTransform, maxDistance, out result);
        ...
        {
            var aabb         = Physics.AabbFrom(triMesh, math.mul(math.inverse(terrainTransform), triMeshTransform));
                                        ...
                                        out ColliderCastResult result)
        {
            ...
            var aabb                          =
                Physics.AabbFrom(triMeshToCast, in casterInTargetSpace, casterInTargetSpace.pos + math.rotate(targetTerrainTransformInverse, castEnd - castStart.pos));
            ...
            var triangle         = PointRayTerrain.CreateLocalTriangle(ref targetTerrain.terrainColliderBlob.Value,
                                                                       ...
                                                                       targetTerrain.scale);
            TriangleTriMesh.DistanceBetween(in triMeshToCast,
                                            in hitTransform,
                                            in triangle,
                                        ...
                                        out ColliderCastResult result)
        {
            ...
            var targetInCasterSpace = math.mul(worldToCasterSpace, targetTriMeshTransform);
            var reverseCastEnd      = targetInCasterSpace.pos + math.rotate(worldToCasterSpace, castReverse);
            var aabb                = Physics.AabbFrom(targetTriMesh, in targetInCasterSpace, reverseCastEnd);
            ...
            var triangle         = PointRayTerrain.CreateLocalTriangle(ref terrainToCast.terrainColliderBlob.Value,
                                                                       ...
                                                                       terrainToCast.scale);
            TriangleTriMesh.DistanceBetween(in targetTriMesh,
                                                                          ...
                                                                          in ColliderDistanceResult distanceResult)
        {
            ...
            return TriangleTriMesh.UnityContactsBetween(in triMesh, in triMeshTransform, in triangle, in terrainTransform, distanceResult.ToFlipped()).ToFlipped();
        }
        ...
        unsafe struct DistanceBetweenAllProcessor<T> : TerrainColliderBlob.IFindTrianglesProcessor where T : unmanaged, IDistanceBetweenAllProcessor
        {
            ...
            public void Execute(ref TerrainColliderBlob blob, int3 triangleHeightIndices, int triangleIndex)
            {
                ...
                var hit                      = TriangleTriMesh.DistanceBetween(in triMesh, in triMeshTransform, in triangle, in terrainTransform, maxDistance, out var tempResult);
        ...
        struct CastProcessor : TerrainColliderBlob.IFindTrianglesProcessor
        {
            ...
            public void Execute(ref TerrainColliderBlob blob, int3 triangleHeightIndices, int triangleIndex)
            {
                ...
                if (TriangleTriMesh.ColliderCast(in triMesh, in castStart, castEnd, in triangle, in terrainTransform, out var hit))
                {

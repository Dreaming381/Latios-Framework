...
namespace Latios.Psyshock
{
    internal static class BoxCompound
    {
        ...
        {
            foreach (var i in new PointRayCompound.CompoundAabbEnumerator(box, boxTransform, compound, compoundTransform, 0f))
            {
                compound.GetScaledStretchedSubCollider(i, out var blobCollider, out var blobTransform);
                if (AreOverlapping(in blobCollider, math.mul(compoundTransform, blobTransform), in box, in boxTransform))
                    return true;
            }
            return false;
        }
        ...
        public static bool WithinDistance(in CompoundCollider compound,
                                          ...
                                          float maxDistance)
        {
            foreach (var i in new PointRayCompound.CompoundAabbEnumerator(box, boxTransform, compound, compoundTransform, maxDistance))
            {
                compound.GetScaledStretchedSubCollider(i, out var blobCollider, out var blobTransform);
                if (WithinDistance(in blobCollider, math.mul(compoundTransform, blobTransform), in box, in boxTransform, maxDistance))
                    return true;
            }
            return false;
        }
                                        ...
                                        out ColliderCastResult result)
        {
            ...
            result          = default;
            result.distance = float.MaxValue;
            if (AreOverlapping(in targetCompound, in targetCompoundTransform, in boxToCast, in castStart))
            {
                return false;
            }
            foreach (var i in new PointRayCompound.CompoundAabbEnumerator(Physics.AabbFrom(boxToCast, in castStart, castEnd), targetCompound, targetCompoundTransform))
            {
                                        ...
                                        out ColliderCastResult result)
        {
            ...
            result          = default;
            result.distance = float.MaxValue;
            if (AreOverlapping(in compoundToCast, in castStart, in targetBox, in targetBoxTransform))
            {
                return false;
            }
            var boxSweptAabb = Physics.AabbFrom(targetBox, targetBoxTransform, targetBoxTransform.pos - (castEnd - castStart.pos));
            foreach (var i in new PointRayCompound.CompoundAabbEnumerator(boxSweptAabb, compoundToCast, castStart))
            {
                                           ...
                                           in RigidTransform boxTransform)
        {
            switch (collider.type)
            {
                case ColliderType.Sphere:
                    return SphereBox.AreOverlapping(in box, in boxTransform, in collider.m_sphere, in colliderTransform);
                case ColliderType.Capsule:
                    return CapsuleBox.AreOverlapping(in box, in boxTransform, in collider.m_capsule, in colliderTransform);
                case ColliderType.Box:
                    return BoxBox.AreOverlapping(in box, in boxTransform, in collider.m_box, in colliderTransform);
                default:
                    return false;
            }
        }
                                           ...
                                           float maxDistance)
        {
            ...
            {
                case ColliderType.Sphere:
                    return SphereBox.WithinDistance(in box, in boxTransform, in collider.m_sphere, in colliderTransform, maxDistance);
                case ColliderType.Capsule:
                    return CapsuleBox.WithinDistance(in box, in boxTransform, in collider.m_capsule, in colliderTransform, maxDistance);
                case ColliderType.Box:
                    return BoxBox.WithinDistance(in box, in boxTransform, in collider.m_box, in colliderTransform, maxDistance);
                default:
                    return false;
            }
        }

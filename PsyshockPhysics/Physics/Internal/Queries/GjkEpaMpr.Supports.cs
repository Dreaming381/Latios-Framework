...
namespace Latios.Psyshock
{
    internal static partial class SpatialInternal
    {
        ...
        private static float3 GetSupport(in Collider collider, int id)
        {
        ...
        private static SupportPoint GetSupport(in Collider collider, float3 direction, RigidTransform bInA)
        {
            switch (collider.type)
            {
                case ColliderType.Sphere:
        ...
        private static SupportPoint GetSupport(in Collider colliderA, in Collider colliderB, float3 direction, RigidTransform bInA)
        {
            var a = GetSupport(in colliderA, direction);
            var b = GetSupport(in colliderB, -direction, bInA);
            return new SupportPoint
            {
                pos = a.pos - b.pos,
                id  = (a.id << 16) | b.id
        ...
        private static SupportPoint Get3DSupportFromPlanar(in Collider colliderA, in Collider colliderB, RigidTransform bInA, SupportPoint planarSupport)
        {
        ...
        private static Aabb GetCsoAabb(in Collider colliderA, in Collider colliderB, RigidTransform bInA)
        {
            var aabbA = Physics.AabbFrom(in colliderA, RigidTransform.identity);
            var aabbB = Physics.AabbFrom(in colliderB, in bInA);
            return new Aabb(aabbA.min - aabbB.max, aabbA.max - aabbB.min);
        }

        private static float GetRadialPadding(in Collider collider)
        {
            switch (collider.type)
            {
                case ColliderType.Sphere:
        ...
        private static SupportPoint GetPlanarSupport(in Collider colliderA, in Collider colliderB, float3 direction, RigidTransform bInA, float3 planeNormal)
        {
            var    a          = GetSupport(in colliderA, direction);
            var    b          = GetSupport(in colliderB, -direction, bInA);

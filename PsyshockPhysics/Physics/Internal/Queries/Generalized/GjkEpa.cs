...
namespace Latios.Psyshock
{
    internal static class GjkEpa
    {
        ...
        public static unsafe GjkResult DoGjkEpa(in Collider colliderA, in Collider colliderB, in RigidTransform bInASpace)
        {
            ...
            simplex.a                                     = MinkowskiSupports.GetSupport(colliderA, colliderB, new float3(1f, 0f, 0f), bInASpace);
            ...
            for (; iteration < maxIterations; ++iteration)
            {
                ...
                var newSupportPoint = MinkowskiSupports.GetSupport(colliderA, colliderB, -simplex.originToClosestPointUnscaledDirection, bInASpace);
            ...
            if (penetration)
            {
                ...
                Aabb                        domain                  = MinkowskiSupports.GetCsoAabb(colliderA, colliderB, bInASpace);
                ...
                while (hull.dimension < 3)
                {
                    ...
                    for (int i = 0; i < numSupports; i++)
                    {
                        ...
                        {
                            var vertex = MinkowskiSupports.GetSupport(colliderA, colliderB, support0, bInASpace);
            ...
            result.hitpointOnAInASpace = MinkowskiSupports.GetSupport(colliderA, simplex.a.idA) * coordinates.x +
                                         MinkowskiSupports.GetSupport(colliderA, simplex.b.idA) * coordinates.y +
                                         MinkowskiSupports.GetSupport(colliderA, simplex.c.idA) * coordinates.z +
                                         MinkowskiSupports.GetSupport(colliderA, simplex.d.idA) * coordinates.w;

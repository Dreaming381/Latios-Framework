    ...
    internal static partial class SpatialInternal
    {
                                                ...
                                                out bool somethingWentWrong)
        {
            ...
            if (math.all(initialCsoAabb.max < 0f | initialCsoAabb.min > 0f))
            {
                ...
                bool  hitForward    = SpatialInternal.RaycastAabb(new Ray(0f, normalizedCastDirectionInCasterSpace * castMagnitude), initialCsoAabb, out _);
                bool  hitBackward   = SpatialInternal.RaycastAabb(new Ray(0f, -normalizedCastDirectionInCasterSpace * castMagnitude), initialCsoAabb, out _);
        ...
        internal static bool MprCastNoRoundnessDebug(Collider caster,
                                                     Collider target,
                                                     RigidTransform targetInCasterSpace,
                                                     float3 normalizedCastDirectionInCasterSpace,
                                                     float maxCastDistance,
                                                     ...
                                                     out bool somethingWentWrong)
        {
            ...
            // Third, check if the caster reaches the target laterally using planar MPR
            if (!Mpr.DoPlanarMprDebug(caster, target, targetInCasterSpace, normalizedCastDirectionInCasterSpace, portalA.pos, out var portalB, out var portalC,
                                      ref somethingWentWrong))
            {
                distanceOfImpact = Mpr.k_missDistance;
                return false;
            ...
            // Catch the case where the ray misses the portal triangle. It is frustrating that it happens, but this should catch it.
            Mpr.DoMprPortalSearchDebug(caster, target, slidTargetInCasterSpace, normalizedCastDirectionInCasterSpace, ref portalA, ref portalB, ref portalC,
                                       ref somethingWentWrong);

            UnityEngine.Debug.Log($"Post-search portal: A: {portalA.pos}, {portalA.id}, B: {portalB.pos}, {portalB.id}, C: {portalC.pos}, {portalC.id}");

            float mprDistance = Mpr.DoMprRefine3DDebug(caster,
                                                       target,
                                                       slidTargetInCasterSpace,
                                                       normalizedCastDirectionInCasterSpace,
                                                       portalA,
        ...
        private static class Mpr
        {
                                                ...
                                                ref bool somethingWentWrong)
            {
                ...
                if (math.abs(denom) < math.EPSILON)
                {
                    ...
                    if (math.all(plane.normal == 0f))
                    {
                        ...
                        QueriesLowLevelUtils.SegmentSegment(0f, rayExtents, targetA, targetB - targetA, out var closestA, out _);
                    ...
                    hit.x            = SpatialInternal.RaycastQuad(ray, quadAB, out fractions.x);
                    hit.y            = SpatialInternal.RaycastQuad(ray, quadBC, out fractions.y);
                    hit.z            = SpatialInternal.RaycastQuad(ray, quadCA, out fractions.z);
            ...
            internal static void DoMprPortalSearchDebug(Collider colliderA,
                                                        Collider colliderB,
                                                        RigidTransform bInASpace,
                                                        float3 normalizedSearchDirectionInASpace,
                                                        ref SupportPoint portalA,
            ...
            internal static float DoMprRefine3DDebug(Collider colliderA,
                                                     Collider colliderB,
                                                     RigidTransform bInASpace,
                                                     float3 normalizedSearchDirectionInASpace,
                                                     SupportPoint portalA,
            ...
            internal static bool DoPlanarMprDebug(Collider colliderA,
                                                  Collider colliderB,
                                                  RigidTransform bInASpace,
                                                  float3 planeNormal,
                                                  float3 searchStart,

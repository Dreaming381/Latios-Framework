            ...
            for (int i = transition.startIndex; i <= transition.endIndex; i++)
            {
                switch (transition.scope)
                {
                    case TransitionTextUnitScope.Glyph:
                        SetRotationValue(ref renderGlyphs, i, i,
                                         values[transition.endIndex - i]);
                        break;
                    case TransitionTextUnitScope.Word:
                        SetRotationValue(ref renderGlyphs, glyphMapper.GetGlyphStartIndexAndCountForWord(i).x, glyphMapper.GetGlyphStartIndexAndCountForWord(i).y - 1,
                                         values[transition.endIndex - glyphMapper.GetGlyphStartIndexAndCountForWord(i).x]);
                        break;
                    case TransitionTextUnitScope.Line:
                        SetRotationValue(ref renderGlyphs, glyphMapper.GetGlyphStartIndexAndCountForLine(i).x, glyphMapper.GetGlyphStartIndexAndCountForLine(i).y - 1,

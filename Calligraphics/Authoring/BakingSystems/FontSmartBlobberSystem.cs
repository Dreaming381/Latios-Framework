...
namespace Latios.Calligraphics.Authoring.Systems
{
    ...
    public partial class FontSmartBlobberSystem : SystemBase
    {
        ...
        public static unsafe BlobAssetReference<FontBlob> BakeFont(FontAsset font, Material material)
        {
            ...
            var glyphCount = 0;
            for (int i = 0, length = font.characterTable.Count; i < length; i++)
                if (font.characterTable[i] != null) glyphCount++;
            
            BlobBuilderArray<GlyphBlob>      glyphBuilder    = builder.Allocate(ref fontBlobRoot.characters, glyphCount);
            ...
            for (int i = 0, length = glyphPairAdjustmentsSource.Count;  i < length; i++)
            {

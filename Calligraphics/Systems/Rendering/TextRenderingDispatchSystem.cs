...
namespace Latios.Calligraphics.Rendering.Systems
{
    ...
    public partial class TextRenderingDispatchSystem : CullingComputeDispatchSubSystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            broker.InitializePersistentBuffer(kGlyphsBufferID, 128, 4, GraphicsBuffer.Target.Raw, copyByteAddressShader);
        ...
        protected override IEnumerable<bool> UpdatePhase()
        {
            while (true)
            {
                ...
                var uploadBuffer = graphicsBroker.GetUploadBuffer(kGlyphsUploadID, math.max(requiredUploadBufferSize.Value, 128) * 24);
                var metaBuffer   = graphicsBroker.GetMetaUint4UploadBuffer((uint)payloads.Length);
                ...
                var persistentBuffer = graphicsBroker.GetPersistentBuffer(kGlyphsBufferID,

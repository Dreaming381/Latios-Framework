...
namespace Latios.Calligraphics.RichText.Parsing
{
    internal static class RichTextParser
    {
        ...
        internal static bool ValidateHtmlTag(
            ...
            ref FixedList512Bytes<RichTextAttribute> richTextAttributes)   //this is just a cache to avoid allocation 
        {
            richTextAttributes.Clear();
            int tagCharCount = 0;
            int tagByteCount = 0;
            int startByteIndex = enumerator.CurrentByteIndex;
            ParserState attributeFlag = ParserState.Zero;

            int attributeIndex = richTextAttributes.Length;
            richTextAttributes.Add(RichTextAttribute.Empty);
            ref var currentAttribute = ref richTextAttributes.ElementAt(attributeIndex);
            TagValueType tagValueType = currentAttribute.valueType = TagValueType.None;
            TagUnitType tagUnitType = currentAttribute.unitType = TagUnitType.Pixels;
            ...
            while (enumerator.MoveNext() && (unicode = enumerator.Current) != Unicode.BadRune && unicode != '<')
            {
                ...
                if (attributeFlag == ParserState.One)
                {
                    if (tagValueType == TagValueType.None)
                    {
                        ...
                        if (unicode == '+' || unicode == '-' || unicode == '.' || Unicode.Rune.IsDigit(unicode))
                        {
                            ...
                            tagValueType = currentAttribute.valueType = TagValueType.NumericalValue;
                            currentAttribute.valueStartIndex = enumerator.CurrentByteIndex - unicode.LengthInUtf8Bytes();
                            currentAttribute.valueLength += byteCount;
                        }
                        else if (unicode == '#')
                        {
                            tagUnitType = TagUnitType.Pixels;
                            tagValueType = currentAttribute.valueType = TagValueType.ColorValue;
                            currentAttribute.valueStartIndex = enumerator.CurrentByteIndex - unicode.LengthInUtf8Bytes();
                            currentAttribute.valueLength += byteCount;
                        }
                        else if (unicode == '"')
                        {
                            tagUnitType = TagUnitType.Pixels;
                            tagValueType = currentAttribute.valueType = TagValueType.StringValue;
                            currentAttribute.valueStartIndex = enumerator.CurrentByteIndex;
                        }
                        else
                        {
                            tagUnitType = TagUnitType.Pixels;
                            tagValueType = currentAttribute.valueType = TagValueType.StringValue;
                            currentAttribute.valueStartIndex = enumerator.CurrentByteIndex - unicode.LengthInUtf8Bytes();
                            currentAttribute.valueHashCode = (currentAttribute.valueHashCode << 5) + currentAttribute.valueHashCode ^ unicode.value;
                            currentAttribute.valueLength += byteCount;
                        }
                    }
                    else
                    {
                        if (tagValueType == TagValueType.NumericalValue)
                        {
                            ...
                            {
                                attributeFlag = ParserState.Two;
                                tagValueType = TagValueType.None;

                                switch (unicode.value)
                                {
                                    case 'e':
                                        currentAttribute.unitType = tagUnitType = TagUnitType.FontUnits;
                                        break;
                                    case '%':
                                        currentAttribute.unitType = tagUnitType = TagUnitType.Percentage;
                                        break;
                                    default:
                                        currentAttribute.unitType = tagUnitType = TagUnitType.Pixels;
                                        break;
                                }

                                attributeIndex += 1;
                                richTextAttributes.Add(RichTextAttribute.Empty);
                                currentAttribute = ref richTextAttributes.ElementAt(attributeIndex);
                            }
                            else if (attributeFlag != ParserState.Two)
                            {
                                currentAttribute.valueLength += byteCount;
                            }
                        }
                ...
                if (unicode == '=') // '='
                    attributeFlag = ParserState.One;

                // Compute HashCode for the name of the attribute
                if (attributeFlag == ParserState.Zero && unicode == ' ')
                {
                ...
                if (attributeFlag == ParserState.Zero)
                    currentAttribute.nameHashCode = (currentAttribute.nameHashCode << 3) - currentAttribute.nameHashCode + unicode.value;

                if (attributeFlag == ParserState.Two && unicode == ' ')
                    attributeFlag = ParserState.Zero;
            }
            ...
            ref var firstAttribute = ref richTextAttributes.ElementAt(0);
            ...
            if (textConfiguration.tag_NoParsing && (firstAttribute.nameHashCode != 53822163 && firstAttribute.nameHashCode != 49429939))
                return false;
            else if (firstAttribute.nameHashCode == 53822163 || firstAttribute.nameHashCode == 49429939)
            {
            ...
            {
                firstAttribute.tagType = RichTextTagType.Color;
            ...
            else
            {
                ...
                switch (firstAttribute.nameHashCode)
                {
                    case 98: // <b>
                    case 66: // <B>
                    ...
                    case 105: // <i>
                    case 73: // <I>
                        firstAttribute.tagType = RichTextTagType.Italic;
                        textConfiguration.m_FontStyleInternal |= FontStyles.Italic;
                        textConfiguration.m_fontStyleStack.Add(FontStyles.Italic);

                        if (richTextAttributes.Length > 0 && richTextAttributes[1].nameHashCode == 276531 || richTextAttributes[1].nameHashCode == 186899)
                        {

                            // Reject tag if value is invalid.
                            calliString.GetSubString(ref textConfiguration.m_htmlTag, richTextAttributes[1].valueStartIndex, richTextAttributes[1].valueLength);
                            ...
                            if (textConfiguration.m_ItalicAngle < -180 || textConfiguration.m_ItalicAngle > 180) return false;
                        }
                        else
                    ...
                    case 83: // <S>
                        ...
                        if (richTextAttributes.Length > 0 && richTextAttributes[1].nameHashCode == 281955 || richTextAttributes[1].nameHashCode == 192323)
                        {
                            calliString.GetSubString(ref textConfiguration.m_htmlTag, richTextAttributes[1].valueStartIndex, richTextAttributes[1].valueLength);
                            charCount = richTextAttributes[1].valueLength - richTextAttributes[1].valueStartIndex;
                            textConfiguration.m_strikethroughColor = HexCharsToColor(textConfiguration.m_htmlTag, charCount);
                            textConfiguration.m_strikethroughColor.a = textConfiguration.m_htmlColor.a < textConfiguration.m_strikethroughColor.a ? (byte)(textConfiguration.m_htmlColor.a) : (byte)(textConfiguration.m_strikethroughColor.a);
                        }
                        else
                    ...
                    case 85: // <U>
                        ...
                        if (richTextAttributes.Length > 0 && richTextAttributes[1].nameHashCode == 281955 || richTextAttributes[1].nameHashCode == 192323)
                        {
                            ...
                            textConfiguration.m_underlineColor.a = textConfiguration.m_htmlColor.a < textConfiguration.m_underlineColor.a ? (byte)(textConfiguration.m_htmlColor.a) : (byte)(textConfiguration.m_underlineColor.a);
                        }
                        else
                    ...
                    case 30245: // <MARK>
                        ...
                        Calli_Offset highlightPadding = Calli_Offset.zero;

                        // Handle Mark Tag and potential attributes
                        for (int i = 0; i < richTextAttributes.Length && richTextAttributes[i].nameHashCode != 0; i++)
                        {
                            int nameHashCode = richTextAttributes[i].nameHashCode;
                            ...
                            switch (nameHashCode)
                            {
                                ...
                                case 43045:
                                case 30245:
                                    if (richTextAttributes[i].valueType == TagValueType.ColorValue)
                                    {
                                        //is this a bug in TMP Pro? -->should be richTextAttributes[i] and not firstAttribute
                                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                                        charCount = firstAttribute.valueLength - firstAttribute.valueStartIndex;
                                ...
                                // Color attribute
                                case 281955:
                                    calliString.GetSubString(ref textConfiguration.m_htmlTag, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength);
                                    charCount = richTextAttributes[i].valueLength - richTextAttributes[i].valueStartIndex;
                                    highlightColor = HexCharsToColor(textConfiguration.m_htmlTag, charCount);
                                    break;

                                // Padding attribute
                                case 15087385:
                                    //int paramCount = GetAttributeParameters(calliString, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength, ref m_attributeParameterValues);
                                    //if (paramCount != 4) return false;

                                    //highlightPadding = new Calli_Offset(m_attributeParameterValues[0], m_attributeParameterValues[1], m_attributeParameterValues[2], m_attributeParameterValues[3]);
                    ...
                    case 4728: // <SUB>
                        ...
                        fontScale = (textConfiguration.m_currentFontSize / currenFont.pointSize * currenFont.scale * (baseConfiguration.isOrthographic ? 1 : 0.1f));
                    ...
                    case 2012149182: // <FONT-WEIGHT>
                        firstAttribute.tagType = RichTextTagType.FontWeight;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 4556: // <POS>
                        firstAttribute.tagType = RichTextTagType.Position;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 11642281: // <VOFFSET>
                        firstAttribute.tagType = RichTextTagType.VerticalOffset;
                        // Reject tag if value is invalid.
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 45545: // <size=>
                    case 32745: // <SIZE>
                        firstAttribute.tagType = RichTextTagType.Size;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    //case 41311: // <font=xx>
                    //case 28511: // <FONT>
                    //    int fontHashCode = firstAttribute.valueHashCode;
                    //    int materialAttributeHashCode = richTextAttributes[1].nameHashCode;
                    //    int materialHashCode = richTextAttributes[1].valueHashCode;
                    ...
                    //        tempFont = OnFontAssetRequest?.Invoke(fontHashCode, new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));

                    //        if (tempFont == null)
                    //        {
                    //            // Load Font Asset
                    //            tempFont = Resources.Load<TMP_FontAsset>(TMP_Settings.defaultFontAssetPath + new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));
                    //        }
                    ...
                    //        {
                    //            // Load new material
                    //            tempMaterial = Resources.Load<Material>(TMP_Settings.defaultFontAssetPath + new string(calliString, richTextAttributes[1].valueStartIndex, richTextAttributes[1].valueLength));
                    ...
                    //    materialHashCode = firstAttribute.valueHashCode;
                    ...
                    //    {
                    //        // Load new material
                    //        tempMaterial = Resources.Load<Material>(TMP_Settings.defaultFontAssetPath + new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));
                    ...
                    case 230446: // <SPACE>
                        firstAttribute.tagType = RichTextTagType.Space;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 186622: // <ALPHA>
                        firstAttribute.tagType = RichTextTagType.Alpha;
                        if (firstAttribute.valueLength != 3) return false;
                    ...
                    //        m_textInfo.linkInfo[index].hashCode = firstAttribute.valueHashCode;
                    //        m_textInfo.linkInfo[index].linkTextfirstCharacterIndex = m_characterCount;

                    //        m_textInfo.linkInfo[index].linkIdFirstCharacterIndex = startIndex + firstAttribute.valueStartIndex;
                    //        m_textInfo.linkInfo[index].linkIdLength = firstAttribute.valueLength;
                    //        m_textInfo.linkInfo[index].SetLinkID(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                    //    }
                    //    return true;
                    //case 155913: // </link>
                    //case 143113: // </LINK>
                    ...
                    case 275917: // <align=>
                    case 186285: // <ALIGN>
                        switch (firstAttribute.valueHashCode)
                        {
                    ...
                    case 237918: // <WIDTH>
                        firstAttribute.tagType = RichTextTagType.Width;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    //    firstAttribute.tagType = RichTextTagType.Gradient;
                    //    calliString.GetSubString(ref richtextAdjustments.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                    //    int gradientPresetHashCode = firstAttribute.valueHashCode;
                    ...
                    //    //    {
                    //    //        tempColorGradientPreset = Resources.Load<TMP_ColorGradient>(TMP_Settings.defaultColorGradientPresetsPath + new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));
                    //    //    }
                    ...
                    //    // Check Attributes
                    //    for (int i = 1; i < richTextAttributes.Length && richTextAttributes[i].nameHashCode != 0; i++)
                    //    {
                    //        // Get attribute name
                    //        int nameHashCode = richTextAttributes[i].nameHashCode;
                    ...
                    //            case 45819: // tint
                    //            case 33019: // TINT
                    //                calliString.GetSubString(ref richtextAdjustments.m_htmlTag, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength);
                    ...
                    case 1356515: // <CSPACE>
                        firstAttribute.tagType = RichTextTagType.CharacterSpacing;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 7513474: // </cspace>
                    case 6886018: // </CSPACE>
                        if (!textConfiguration.m_isParsingText) return true;
                    ...
                    case 1524585: // <MSPACE>
                        firstAttribute.tagType = RichTextTagType.MonoSpace;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 1441524: // <INDENT>
                        firstAttribute.tagType = RichTextTagType.Indent;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case -842656867: // <LINE-INDENT>
                        firstAttribute.tagType = RichTextTagType.LineIndent;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    //case 1619421: // <SPRITE>
                    //    int spriteAssetHashCode = firstAttribute.valueHashCode;
                    //    TMP_SpriteAsset tempSpriteAsset;
                    //    m_spriteIndex = -1;

                    //    // CHECK TAG FORMAT
                    //    if (firstAttribute.valueType == TagValueType.None || firstAttribute.valueType == TagValueType.NumericalValue)
                    //    {
                    ...
                    //            {
                    //                //
                    //                tempSpriteAsset = OnSpriteAssetRequest?.Invoke(spriteAssetHashCode, new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));

                    //                if (tempSpriteAsset == null)
                    //                    tempSpriteAsset = Resources.Load<TMP_SpriteAsset>(TMP_Settings.defaultSpriteAssetPath + new string(calliString, firstAttribute.valueStartIndex, firstAttribute.valueLength));
                    //            }
                    ...
                    //    if (firstAttribute.valueType == TagValueType.NumericalValue) // <sprite=index>
                    //    {
                    ...
                    //    for (int i = 0; i < richTextAttributes.Length && richTextAttributes[i].nameHashCode != 0; i++)
                    //    {
                    //        //Debug.Log("Attribute[" + i + "].nameHashCode=" + richTextAttributes[i].nameHashCode + "   Value:" + ConvertToFloat(ref richtextAdjustments.m_htmlTag, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength));
                    //        int nameHashCode = richTextAttributes[i].nameHashCode;
                    //        int index = 0;
                    ...
                    //                m_currentSpriteAsset = TMP_SpriteAsset.SearchForSpriteByHashCode(m_currentSpriteAsset, richTextAttributes[i].valueHashCode, true, out index);
                    ...
                    //                index = (int)ConvertToFloat(ref richtextAdjustments.m_htmlTag, richTextAttributes[1].valueStartIndex, richTextAttributes[1].valueLength);
                    ...
                    //            case 45819: // tint
                    //            case 33019: // TINT
                    //                m_tintSprite = ConvertToFloat(ref richtextAdjustments.m_htmlTag, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength) != 0;
                    //                break;
                    //            case 281955: // color=#FF00FF80
                    //            case 192323: // COLOR
                    //                m_spriteColor = HexCharsToColor(calliString, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength);
                    //                break;
                    //            case 39505: // anim="0,16,12"  start, end, fps
                    //            case 26705: // ANIM
                    //                        //Debug.Log("Start: " + richTextAttributes[i].valueStartIndex + "  Length: " + richTextAttributes[i].valueLength);
                    //                int paramCount = GetAttributeParameters(calliString, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength, ref m_attributeParameterValues);
                    ...
                    case 1482398: // <MARGIN>
                                  // Check value type
                        switch (firstAttribute.valueType)
                        {
                            case TagValueType.NumericalValue:
                                firstAttribute.tagType = RichTextTagType.Margin;
                                calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                                // Reject tag if value is invalid.
                                if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                                    return false;
                                ...
                                switch (tagUnitType)
                                {
                                    ...
                                    case TagUnitType.FontUnits:
                                        ...
                                        break;
                                    case TagUnitType.Percentage:
                                        textConfiguration.m_marginLeft = (textConfiguration.m_marginWidth - (textConfiguration.m_width != -1 ? textConfiguration.m_width : 0)) * value / 100;
                                        break;
                                }
                            ...
                            case TagValueType.None:
                                for (int i = 1; i < richTextAttributes.Length && richTextAttributes[i].nameHashCode != 0; i++)
                                {
                                    currentAttribute = ref richTextAttributes.ElementAt(i);
                                    // Get attribute name
                                    int nameHashCode = richTextAttributes[i].nameHashCode;
                                    ...
                                    switch (nameHashCode)
                                    {
                                        case 42823:  // <margin left=value>
                                            currentAttribute.tagType = RichTextTagType.Margin;
                                            calliString.GetSubString(ref textConfiguration.m_htmlTag, currentAttribute.valueStartIndex, currentAttribute.valueLength);
                                            // Reject tag if value is invalid.
                                            if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                                                return false;

                                            switch (richTextAttributes[i].unitType)
                                            {
                                                case TagUnitType.Pixels:
                                                    textConfiguration.m_marginLeft = value * (baseConfiguration.isOrthographic ? 1 : 0.1f);
                                                    break;
                                        ...
                                        case 315620: // <margin right=value>
                                            ...
                                            switch (richTextAttributes[i].unitType)
                                            {
                                                ...
                                                case TagUnitType.FontUnits:
                                                    ...
                                                    break;
                                                case TagUnitType.Percentage:
                                                    textConfiguration.m_marginRight = (textConfiguration.m_marginWidth - (textConfiguration.m_width != -1 ? textConfiguration.m_width : 0)) * value / 100;
                                                    break;
                                            }
                    ...
                    case -842693512: // <LINE-HEIGHT>
                        firstAttribute.tagType = RichTextTagType.LineHeight;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    //case 1913798: // <action>
                    //case 1286342: // <ACTION>
                    //    int actionID = firstAttribute.valueHashCode;
                    ...
                    //    //if (m_isParsingText)
                    //    //{
                    //    // TMP_Action action = TMP_Action.GetAction(firstAttribute.valueHashCode);
                    //    //}
                    //    return true;
                    //case 7443301: // </action>
                    //case 6815845: // </ACTION>
                    ...
                    case 1600507: // <ROTATE=xx.x>
                        ...
                        firstAttribute.tagType = RichTextTagType.Rotate;
                        calliString.GetSubString(ref textConfiguration.m_htmlTag, firstAttribute.valueStartIndex, firstAttribute.valueLength);
                        // Reject tag if value is invalid.
                        if (ConvertToFloat(ref textConfiguration.m_htmlTag, out value) != ParseError.None)
                            return false;
                    ...
                    case 317446: // <table>
                    case 227814: // <TABLE>
                        //switch (richTextAttributes[1].nameHashCode)
                        //{
                        //    case 327550: // width
                        //        float tableWidth = ConvertToFloat(ref richtextAdjustments.m_htmlTag, richTextAttributes[1].valueStartIndex, richTextAttributes[1].valueLength);
                    ...
                    case 656: // <TD>
                        ...
                        //for (int i = 1; i < richTextAttributes.Length && richTextAttributes[i].nameHashCode != 0; i++)
                        //{
                        //    switch (richTextAttributes[i].nameHashCode)
                        //    {
                        //        case 327550: // width
                        //            float tableWidth = ConvertToFloat(ref richtextAdjustments.m_htmlTag, richTextAttributes[i].valueStartIndex, richTextAttributes[i].valueLength);
                        ...
                        //            break;
                        //        case 275917: // align
                        //            switch (richTextAttributes[i].valueHashCode)
                        //            {
                        //                case 3774683: // left

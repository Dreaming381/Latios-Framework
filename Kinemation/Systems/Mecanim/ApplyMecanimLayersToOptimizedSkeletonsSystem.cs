                ...
                while (enumerator.NextEntityIndex(out var indexInChunk))
                {
                    ...
                    if (controller.applyRootMotion)
                    {
                        ...
                        for (int i = 0; i < clipWeights.Length; i++)
                        {
                            ...
                            if (hasLooped)
                            {
                                // Todo: I think there's a bug here where if a 0 duration transition with an exit time transitions into a looping clip,
                                // then the full time duration is applied to the looping clip along with the time fragment of the previous clip.
                                // The fix might be as simple as setting hasLooped to false if time == clipWeight.motionTime, but need to test.
